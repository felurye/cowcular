...
import { useBalanceList } from "@/hooks/use-balances";
...
interface ReportBalance {
  ...
  total_by_member: Record<string, number> | null;
}
...
function memberLabel(m: { user: { name: string } | null; external_name: string | null } | null) {
  if (!m) return "?";
...
export default function ReportsPage() {
  ...
  const { data: rawAccounts } = useAccountList(selectedGroupId, undefined, {
    enabled: !!selectedGroupId,
  });
  const { data: rawBalances } = useBalanceList(selectedGroupId);
  const { data: rawTransfers } = useTransferList(selectedGroupId || undefined);
  ...
  const barData = balances.map((b) => ({
    ...
    total: Math.round(Number(b.total_expense) * 100) / 100,
  }));

  const memberBalance: Record<string, number> = {};
  for (const b of balances) {
    if (!b.total_by_member) continue;
    for (const [memberId, amount] of Object.entries(b.total_by_member)) {
  ...
  const memberBalanceRows = Object.entries(memberBalance).map(([memberId, net]) => {
    ...
    const name = member?.user?.name ?? member?.external_name ?? "Desconhecido";
                    ...
                    {memberBalanceRows
                      ...
                      .map((row) => (
                        <tr key={row.memberId} style={{ borderTop: "1px solid var(--line)" }}>
                          <td
                            style={{ padding: "12px 22px", fontWeight: 600, color: "var(--ink)" }}
                          >
                            {row.name}
                          </td>
                          <td
              ...
              {transfers.length === 0 ? (
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13.5 }}>
                  ...
                  <tbody>
                    {transfers.map((t) => {
                      const isMine =
                        t.from_member.user_id === user?.id || t.to_member.user_id === user?.id;
                      ...
                      return (
                        ...
                        >
                          <td style={{ padding: "11px 18px", color: "var(--ink)" }}>
                            {memberLabel(t.from_member)}
                          </td>
                          <td style={{ padding: "11px 18px", color: "var(--ink)" }}>
                            {memberLabel(t.to_member)}
                          </td>
