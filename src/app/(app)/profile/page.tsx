...
import { useChangePassword, useProfile, useUpdateProfile } from "@/hooks/use-auth-profile";
...
export default function ProfilePage() {
  const { data: rawMe } = useProfile();
  const me = rawMe as ProfileData | undefined;
  ...
  useEffect(() => {
    if (me) {
      setName(me.name ?? "");
      setAvatar(me.avatar ?? "");
      setCurrency(me.default_currency ?? "BRL");
    }
  }, [me]);
  ...
  const updateMutation = useUpdateProfile({
    onSuccess: (data) => {
      ...
      if (user) {
        setUser({ ...user, name: (data as ProfileData).name });
      }
    },
        ...
        <section style={sectionStyle}>
          ...
          >
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              ...
              />
              {avatar && (
                // biome-ignore lint/performance/noImgElement: avatar is a user-supplied URL
                <img
                  src={avatar}
